...
Teacher: Original COCO-Stuff DeepLabV3-ResNet50, 53 classes, FROZEN
Student: DeepLabV3-ResNet50,62 classes,First 53 classifier weights copied from COCO teacher,9 new classifier channels randomly initialized
Output:
...
class CrashSafeLogger(object):
    def __init__(self, log_path="deeplabv3_kd_training_print_output.log"):
        self.terminal = sys.stdout
        # Open in append mode ('a') so restarts don't wipe previous text logs
        self.log_file = open(log_path, "a", encoding="utf-8")
        self.new_line = True

    def write(self, message):
        # Prevent appending timestamps to empty prints, spaces, or standalone newlines
        if self.new_line and message.strip():
            timestamp = f"[{datetime.now().strftime('%Y-%m-%d %H:%M:%S')}] "
            self.terminal.write(timestamp)
            self.log_file.write(timestamp)

        # Tracks if the next message block should get a timestamp prefix
        self.new_line = message.endswith('\n')

        # 1. Print core message text to terminal screen
        self.terminal.write(message)

        # 2. Write core message text to the storage file
        self.log_file.write(message)

        # 3. Flush Python's memory cache
        self.log_file.flush()

        # 4. CRASH PROTECTION: Force Windows to physically commit cache to disk blocks
        try:
...
class IDDIncrementalDataset(Dataset):
    def __init__(
        ...
        self.normalize = T.Compose([T.ToTensor(),T.Normalize( mean=[0.485, 0.456, 0.406], std=[0.229, 0.224, 0.225] ),])

    def __len__(self):
        return len(self.file_stems)

    def find_image(self, stem):
        candidates = [os.path.join(self.images_dir, stem.split("_")[0] + "_leftImg8bit" + self.img_ext),
            os.path.join(self.images_dir, stem + self.img_ext),]
        ...
        for fname in os.listdir(self.images_dir):
            if fname.startswith(prefix):
                ...
                if os.path.isfile(path):
                    return path
        raise FileNotFoundError(   f"Image not found for JSON stem: {stem}"        )

    def build_label(self, json_path, width, height):
        label_img = Image.new( "L",(width, height), color=IGNORE_INDEX)
...
def knowledge_distillation_loss( student_logits, teacher_logits, valid_labels, temperature=2.0, confidence_threshold=0.60):
    """ KD between: teacher: 53 classes student: first 53 of 62 classes
    KD is applied primarily where IDD has no direct semantic supervision.  Teacher confidence filtering avoids forcing the student to imitate low-confidence teacher predictions.
    """
...
def evaluate( student,  val_loader, device):
    ...
    avg_val_loss = (total_loss / max(batches, 1)).item()
    return ( avg_val_loss, miou, old_miou, new_miou, iou )
...
def train_incremental( idd_root, coco_checkpoint_path, train_split="train", val_split="val", epochs=40,
    ...
    if not log_file_exists:
        writer.writerow(["epoch", "train_total_loss", "train_ce_loss", "train_kd_loss", "val_loss", "val_miou", "old_53_miou", "new_9_miou", "epoch_time_sec",])
    best_miou = -1.0
    ...
    for epoch in range(1, epochs + 1):
        ...
        for step, (images,labels) in enumerate(train_loader):
            ...
            with torch.amp.autocast( "cuda", enabled=use_cuda ):
                ...
                loss, ce_loss, kd_loss = (compute_total_loss( student_logits=student_output, teacher_logits=teacher_output, labels=labels,
            ce_criterion=ce_criterion, kd_lambda=kd_lambda, temperature=kd_temperature, confidence_threshold=kd_confidence ))
        ...
        avg_ce_loss = ( total_ce_loss / n_train ).item()
        avg_kd_loss = ( total_kd_loss / n_train ).item()
        ...
        writer.writerow([ epoch, round(avg_train_loss, 6), round(avg_ce_loss, 6), round(avg_kd_loss, 6), round(val_loss, 6),
...
if __name__ == "__main__":
    print_final_mapping()
    sys.stdout = CrashSafeLogger("deeplabv3_kd_training_print_output.log")
    train_incremental(
        idd_root="../idd_part_2", coco_checkpoint_path= "deeplabv3_coco_52ktrn_10epch_stuff_val_best.pt", train_split="train",
        val_split="val", epochs=10, batch_size=4, image_size=512, lr_backbone=5e-6, lr_aspp=5e-5, lr_head=1e-3,
        kd_lambda=1.0, kd_temperature=2.0, kd_confidence=0.60, patience=7, min_delta=0.001, device="cuda",
